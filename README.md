...
- 按需解析官方 Telemetry 事件流，生成击杀时间线、伤害/承伤排行、伤害来源和玩家高亮事件
- 查看武器熟练度和生存熟练度数据
- 按地区、赛季和模式浏览全球排行榜
...
| 文件 | 作用 |
| --- | --- |
| `docker-compose.yml` | 编排前端 nginx 容器和后端 Node.js 容器 |
| `backend/Dockerfile` | 构建后端生产镜像，使用 `npm ci --omit=dev` 安装依赖 |
| `frontend/Dockerfile` | 构建前端静态资源，并用 nginx 托管 |
| `frontend/nginx.conf` | 代理 `/api`、`/health`，并支持前端路由 history fallback |
| `backend/.dockerignore`、`frontend/.dockerignore` | 排除依赖、构建产物、缓存和本地密钥 |
...
```bash
node --test test/docker-config.test.cjs
```
